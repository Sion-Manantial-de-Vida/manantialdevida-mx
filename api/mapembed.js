...
function toEmbed(u) {
  let m;
  // Preferir el NOMBRE del lugar: así el mapa muestra la ficha y "Abrir en Maps"
  // abre esa ficha (no unas coordenadas sueltas).
  ...
  if ((m = u.match(/!3d(-?\d+\.\d+)!4d(-?\d+\.\d+)/))) return coord(m[1], m[2]);
  if ((m = u.match(/@(-?\d+\.\d+),(-?\d+\.\d+)/))) return coord(m[1], m[2]);
  return null;
}
